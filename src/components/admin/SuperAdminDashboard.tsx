...
import { LICENSE_PLANS } from '../../data/saasData';
import { formatCurrency } from '../../services/financials';
import { LicenseTier, Tenant, TenantFeatures, TenantStatus } from '../../types';
...
export const SuperAdminDashboard: React.FC = () => {
  ...
  const handlePlanChange = (planTier: LicenseTier) => {
    setNewPlan(planTier);
    const plan = LICENSE_PLANS.find((p) => p.tier === planTier);
    if (plan) {
  ...
  const handleProvisionTenant = (e: React.FormEvent) => {
    ...
    const planDetails = LICENSE_PLANS.find((p) => p.tier === newPlan);
    ...
    addTenant({
      ...
      storageUsedGB: 0.1,
      storageLimitGB: planDetails?.storageGB || 500,
    ...
    >
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs text-blue-400 font-semibold font-num">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      const plan = LICENSE_PLANS.find((p) => p.tier === newTier);
                      ...
                      setSelectedTenant((prev) =>
                        prev
                          ? {
                              ...
                              plan: newTier,
                              monthlyPriceINR:
                                plan?.monthlyPriceINR || prev.monthlyPriceINR,
                              maxSeats: plan?.maxSeatsIncluded || prev.maxSeats,
                            }
                          : null
                      );
                    }}
                  ...
                  >
                    <option value="STARTER">STARTER (₹25,000/mo)</option>
                    <option value="PROFESSIONAL">PROFESSIONAL (₹75,000/mo)</option>
                    <option value="ENTERPRISE">ENTERPRISE (₹1,50,000/mo)</option>
                    <option value="SOVEREIGN">SOVEREIGN (₹3,50,000/mo)</option>
                  </select>
                </div>
                ...
                <div>
                  ...
                  <span className="text-[10px] text-slate-400 mt-1 block">
                    Currently allocated: {selectedTenant.seatsAllocated} seats
                  </span>
                </div>
              </div>
